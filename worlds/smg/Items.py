...
from worlds.smg.Types import ItemData, ChapterType, SMGItem, chapter_type_to_name
from worlds.smg.Locations import get_total_locations
...
def create_itempool(world: "SMGWorld") -> List[Item]:
    ...
    for chapter in smg_chapters.keys():
...
def create_item(world: "SMGWorld", name: str) -> Item:
    data = item_table[name]
    return SMGItem(name, data.classification, data.ap_code, world.player)

# Another generic function. For creating a bunch of items at once!
def create_multiple_items(world: "SMGWorld", name: str, count: int,
                          item_type: ItemClassification = ItemClassification.progression) -> List[Item]:
    data = item_table[name]
    itemlist: List[Item] = []

    for i in range(count):
        itemlist += [SMGItem(name, item_type, data.ap_code, world.player)]

    return itemlist

# Finally, where junk items are created
def create_junk_items(world: "SMGWorld", count: int) -> List[Item]:
