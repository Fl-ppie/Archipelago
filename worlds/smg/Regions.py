...
from worlds.smg.Types import SMGLocation
from worlds.smg.Locations import location_table, is_valid_location
...
def create_region(world: "SMGWorld", name: str) -> Region:
    ...
    for (key, data) in location_table.items():
        if data.region == name:
            if not is_valid_location(world, key):
                continue
            location = SMGLocation(world.player, key, data.ap_code, reg)
...
def create_region_and_connect(world: "SMGWorld",
